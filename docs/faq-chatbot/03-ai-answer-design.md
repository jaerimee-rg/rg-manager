...
모델은 **질문에 답이 되는 FAQ를 고르기만** 하고, 학부모에게 나가는 문장은 서버가
등록된 답변 원문을 그대로 넣는다 (`composeAnswer`).

- 응답 스키마에 `answer` 필드를 두지 않는다. 모델이 문장을 쓸 여지를 남기면
  요약·재작성이 일어나 줄바꿈과 표현이 달라진다.
- 모델이 고른 id 중 실제로 존재하는 것만 남기고(`pickFaqIds`), 최대 2개까지 사용한다.
  없는 id 를 골랐다면 근거가 없는 것이므로 버리고 `answered=false` 가 된다.
- 여러 개일 때는 사이에 빈 줄만 넣고 각 원문은 다듬지 않는다 (trim 하지 않는다).
- 말풍선은 `white-space: pre-wrap` 이라 줄바꿈이 그대로 보인다.
